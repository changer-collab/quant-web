...
it('strategies carry stable ids and localized labels', () => {
  ...
  assert.equal(englishStrategies[0].name, 'Ping An Bank');
  assert.equal(chineseStrategies[0].name, '平安银行');
  assert.equal(englishStrategies[0].type, '金融 Traditional Quant Strategy');
  assert.equal(chineseStrategies[0].type, '金融 传统量化策略');
  assert.equal(englishStrategies[1].type, '新能源 HFT Strategy');
  assert.equal(chineseStrategies[1].type, '新能源 高频策略');
  assert.equal(englishStrategies[2].type, '消费 AI Quant Strategy');
  assert.equal(chineseStrategies[2].type, '消费 AI 量化策略');
});
...
it('research reports inherit selected strategy and mode metrics', () => {
  ...
  assert.equal(report.strategyName, 'CATL');
  assert.equal(report.title, 'Report: CATL');
...
it('mock jobs are localized', () => {
  assert.equal(getJobs('en')[0].name, 'Backtest: Ping An Bank');
  assert.equal(getJobs('en')[0].state, 'Running');
  assert.equal(getJobs('zh')[0].name, '回测：平安银行');
