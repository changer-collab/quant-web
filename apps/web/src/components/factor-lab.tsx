...
function FactorEvalPanel({
  ...
  return (
    <section className={s.evalPanel}>
      ...
      {activeTab === 'icAnalysis' && <IcCurveChart icSeries={result.icSeries} />}
      {activeTab === 'sorting' && <GroupReturnGrid groupReturns={result.groupReturns} />}
      {activeTab === 'regression' && <LayerTable layerSummary={result.layerSummary} />}
    </section>
  );
}
