...
- AI 引擎 Agent：负责 `packages/ai-engine`。AI 引擎只做特征提取、训练编排（委托算法层）、预测编排（委托算法层）、模型注册表管理、报告分析文本生成。
...
- 策略库 Agent：负责 `packages/strategies`。AI 预测策略（注册名 `ai_predictor`，类名 `AIModelStrategy`）通过 `artifact_id` 从 ai-engine 模型注册表加载 ModelArtifact，调用 algorithms 包的 SignalGenerator 生成 MLSignal，不直接持有模型对象。
...
- API Agent 只做 HTTP 入口和轻量业务编排，不做数据清洗、回测计算、模型训练；策略研究沉淀的过程、事件、候选状态和 Git 游标由 API 持久化，API 不读取或写入知识库文件。
- Worker Agent 只做异步任务编排，不实现策略、撮合、模型算法和数据清洗；策略研究沉淀阶段一由 Worker 扫描 Git 摘要、在实际回测前后上报事件，不直接写 API SQLite 或知识库。
...
- AI 引擎 Agent 只做特征提取、训练编排（委托算法层）、预测编排（委托算法层）、模型注册表管理、报告分析文本生成。报告分析文本生成当前作为 AI 引擎的子职责（放在 `report_analysis/` 子模块），后续要解耦为独立模块。原描述中的标签、训练、预测——标签类型（LabelType）迁移到算法层，训练/predict 委托 Algorithm ABC。
...
- API Agent 为 Worker 提供研究事件与 Git 游标内部接口；API 依据明确策略归属聚合 collecting 研究过程，无法归属的 Git 事件保持待归类。
- Worker Agent 可以编排因子批量计算和因子评估任务，调用 AI 引擎做特征提取。
- Worker Agent 的 Git 首次扫描只保存当前 HEAD 为游标，不回灌历史提交；后续按提交哈希去重并在每条提交保存后推进游标。
...
packages/factor-lab -> packages/algorithms（后续非线因子扩展时引入）
...
packages/strategies -> packages/algorithms（仅 AI 预测策略消费 ModelArtifact/MLSignal/AlgorithmRegistry）
...
- AI 引擎 Agent：不做回测撮合，不做实盘执行；特征提取可被因子挖掘复用，但因子评估指标的计算不是 AI 引擎的职责。训练/预测委托算法层 Algorithm ABC，标签类型（LabelType）已迁移到算法层。报告分析文本生成放在 `report_analysis/` 子模块，接口设计为"输入 dict，输出文本"，不依赖 BacktestResult 等业务类型，避免跨包类型耦合。当前用规则引擎+模板生成，预留 LLM 接口。
- 算法层 Agent：算法包无 HTTP/CLI 入口，通过 strategy-runtime CLI 延迟导入被调用；算法层仅依赖 strategy-runtime（引用 StrategyCategory/StrategySubcategory）+ 第三方 ML 库（延迟导入）；命名冲突规避：算法层信号类型命名为 `MLSignal`，避免与 strategy-runtime 的 `Signal` 枚举冲突。
...
- 策略库 Agent：策略不直接依赖网站后端；AI 预测策略（ai_predictor / `AIModelStrategy`）可依赖 algorithms 包加载 ModelArtifact 并通过 SignalGenerator 生成 MLSignal，但不在策略库内训练模型。
