 ...
 *
 * AKShare 是 Python 库，Node.js 通过 python3 桥接脚本调用。
 * 桥接脚本接收 JSON 参数，输出 JSON 数组到 stdout。
 *
 * 依赖：
 * - Python 3.8+ 且已安装 akshare（pip install akshare）
 * - 可通过 extra.pythonPath 指定 Python 路径，默认 python3
 */
...
try:
  ...
  if "date" in df.columns:
    ...
    df["timestamp"] = df["datetime"].apply(lambda x: int(__import__("datetime").datetime.strptime(str(x)[:19],"%Y-%m-%d %H:%M:%S").timestamp()*1000))
  df["symbol"] = "${symbol}"
