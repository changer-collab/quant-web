...
import { DataCenterError } from '../errors.js';
...
export async function createDataCenter(config?: DataCenterConfig): Promise<DataCenter> {
  ...
  // 等待 closing → closed 的简易 promise
  let closeWaitResolve: (() => void) | undefined;
  const originalClose = close;

  function waitForClose(): Promise<void> {
    return new Promise((resolve) => { closeWaitResolve = resolve; });
  }

  // 修补 close 让等待者能收到通知
  const closeWithNotify = async () => {
    await originalClose();
    closeWaitResolve?.();
  };
  ...
  const dc: DataCenter = {
    ...
    [Symbol.asyncDispose]: closeWithNotify,
  };
...
/** 带超时的异步操作 */
function withTimeout(fn: () => void, ms: number): Promise<void> {
  if (ms <= 0) {
    fn();
    return Promise.resolve();
  }
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error(`关闭超时 (${ms}ms)`)), ms);
    try {
      fn();
      clearTimeout(timer);
      resolve();
