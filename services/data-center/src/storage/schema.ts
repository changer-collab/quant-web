...
/** 交易日历表 */
export const tradingCalendars = sqliteTable('trading_calendars', {
  exchange: text('exchange').notNull(),
  ...
  primaryKey({ columns: [table.exchange, table.year] }),
]);

/** 指数成分表 */
export const indexConstituents = sqliteTable('index_constituents', {
...
/** 复权因子表 */
export const adjustmentFactors = sqliteTable('adjustment_factors', {
  symbol: text('symbol').notNull(),
...
export const financialRatios = sqliteTable('financial_ratios', {
  ...
  primaryKey({ columns: [table.symbol, table.asOfDate] }),
]);
...
/** 公告事件表 */
export const announcementEvents = sqliteTable('announcement_events', {
  ...
  index('idx_announce_symbol').on(table.symbol, table.eventTime),
]);
...
export const newsArticles = sqliteTable('news_articles', {
  ...
  index('idx_news_time').on(table.publishTime),
]);

/** 情绪指标表 */
export const sentimentPoints = sqliteTable('sentiment_points', {
  symbol: text('symbol').notNull(),
  timestamp: integer('timestamp').notNull(),
...
/** 宏观数据点表 */
export const macroPoints = sqliteTable('macro_points', {
...
/** L2 盘口快照表 */
export const l2Snapshots = sqliteTable('l2_snapshots', {
  symbol: text('symbol').notNull(),
  timestamp: integer('timestamp').notNull(),
...
export const tradeRecords = sqliteTable('trade_records', {
  ...
  index('idx_traderec_symbol').on(table.symbol, table.timestamp),
]);
...
export const orderRecords = sqliteTable('order_records', {
  ...
  index('idx_orderrec_symbol').on(table.symbol, table.timestamp),
]);

// ─── 基本面：股东人数 ──────────────────────────────────────

/** 股东人数表 */
export const shareholderMetrics = sqliteTable('shareholder_metrics', {
  symbol: text('symbol').notNull(),
  announceDate: integer('announce_date').notNull(),
