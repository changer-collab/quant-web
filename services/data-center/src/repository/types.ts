...
export interface FinancialRatioRepository {
  ...
  query(symbol: string, asOfDate?: number): Promise<FinancialRatio[]>;
}

/** 股东人数存储接口 */
export interface ShareholderMetricsRepository {
  save(metrics: ShareholderMetrics[]): Promise<void>;
  query(symbol: string, start?: number, end?: number): Promise<ShareholderMetrics[]>;
...
export interface ValuationRepository {
  ...
  query(symbol: string, start?: number, end?: number): Promise<ValuationPoint[]>;
}
