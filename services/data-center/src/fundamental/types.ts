...
export interface FundamentalDataProvider {
  ...
  /** 获取财务比率快照（PIT） */
  getFinancialRatios(symbol: string, asOfDate: number): Promise<FinancialRatio[]>;
  /** 获取估值序列 */
  getValuationSeries(symbol: string, start?: number, end?: number): Promise<ValuationPoint[]>;
  /** 获取最新财报（PIT） */
  getLatestReport(symbol: string): Promise<FinancialReport | undefined>;
  /** 获取股东人数序列（PIT） */
  getShareholderMetrics(symbol: string, start?: number, end?: number): Promise<ShareholderMetrics[]>;
}
