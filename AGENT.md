...
- 当前阶段：前端研究原型稳定阶段，数据中心和数据采集器研发完成。
- 当前可运行项目：`apps/web`。
- 当前目标：继续打磨前端研究闭环，不做真实交易。
- 核心闭环：选择策略 -> 配置研究参数 -> 运行回测或训练 -> 查看任务和报告 -> 迭代策略。
- 当前前端闭环支持内存态模式化研究配置摘要，运行后写入任务和报告诊断。
- 后续规划：common(已完成) -> 稳定前端 -> 策略运行时(已完成) -> 数据中心(已完成) -> 数据采集器(已完成) -> 回测引擎 -> 因子实验室 -> 策略库 -> Worker -> API -> 前端对接 -> AI 引擎 -> 高频增强 -> 实盘执行层。
...
- `apps/api`（`@quant/api`）：后续 HTTP API，未实现。
- `apps/worker`（`@quant/worker`）：后续异步任务 Worker，未实现。
- `services/data-center`（`@quant/data-center`）：独立数据中心，SQLite + Drizzle ORM，6 个数据子域类型定义、17 个 SQLite Repository、6 个 Provider，已实现。
...
- `packages/factor-lab`（`@quant/factor-lab`）：因子研发工坊，规划阶段（因子定义 / 计算 / 评估 / 注册）。
- `packages/strategies`（`@quant/strategies`）：后续策略库，未实现。
- `packages/common`（`@quant/common`）：公共类型、配置、错误和工具，已实现核心类型基座。
...
- Turborepo 的 `outputs` 警告：stub 包（api、worker、ai-engine）没有 `dist/` 产物，turbo 会报 "no output files found" 警告，这是预期行为，待包实现后自动消除。
