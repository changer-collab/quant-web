...
- **延迟导入**：CLI 命令通过延迟导入加载下游包（backtest-engine、factor-lab、ai-engine、strategies、data-client），strategy-runtime 本身不依赖它们；`commands/__init__.py` 也必须使用函数内导入，避免导入单个命令时加载 sibling 命令依赖
